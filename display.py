...
class Display:
  ...
  def __init__(self, address, rows, columns, temp_screen_period, scroll_period, pins):
  ...
  def show_volume(self):    
    ...
    # Else, show value in % and the corresponding number of blocks
      ...
      # Append volume value
      self.display_data[skip_lines] += str(self.volume_value) + ' %'
  ...
  def screen_0(self):
    ...
    try:
      temp = self.scroll_row(0, self.char_decode(self.mpd.getData()['artist']))
    except KeyError:
      temp = ''
    ...
    try:
      temp = self.scroll_row(1, self.char_decode(self.mpd.getData()['title']))
    except KeyError:
      temp = ''
  ...
  def screen_1(self):
    ...
    # Get bitrate
    bitrate = str(self.mpd.getData()['bitrate']) + ' kbps'
